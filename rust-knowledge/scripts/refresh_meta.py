...
Both are importable; this file also runs standalone as a non-skippable regen step
(set + verify). load_db.py already stamps the same MAX(wave)-derived pointer, so a
clean regen confirms rather than corrects.
