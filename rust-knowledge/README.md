...
deterministic music law being designed the same way. The KB has four tiers, one research wave each:
...
| **si-jam-sessions** | `wave-04-si-jam-sessions` | Rust for a deterministic music law (first four lanes): score ingest inside a wasm law (SMF, MusicXML, ABC), integer musical time (ticks, tempo maps, samples), the native audio and MIDI host, and crate licences for a shipped MIT/Apache product. Its design lock is unsigned, so each recipe states the lock item it assumes. |
...
1. **Currency**, from an adversarial retrieval verifier. This is Claude Sonnet, a different model from
   the Opus research seat, and it never sees the research seat's reasoning. It opens every cited page
   ...
   [briefs/VERIFIER-BRIEF.md](briefs/VERIFIER-BRIEF.md).
...
- **PIN_PER_STEP: 3.** Every seat is pinned per step. Research is Opus in waves 1-3. In wave 4 it is
  ...
  temperature 0. Verification is Sonnet. The prompts are the committed `briefs/LANE-BRIEF.md`,
  `briefs/GENERATOR-BRIEF.md`, `briefs/lanes.json` and `briefs/VERIFIER-BRIEF.md`. Wave 4's receipts
  hash each prompt and each source excerpt, and record each call's id and cost. The tool schema is the lane contract in the brief plus the check format in
...
- **UNCERTAINTY_GATED_HUMANS: 2.** Humans are asked only about what the gates leave open: refuted or
  shaky recipes, and findings that contradict a dispatch pin. These go to the engine's coordinator with
  a "consistent with / contradicts pin N" line each.
- **EXTERNAL_VERIFIER: 3.** The generator never verifies itself. Prose claims go to a different model
  that retrieves the live page; in wave 4 that is also a different model family. Code claims go to the compiler, which is not a model. Any arXiv or DOI
