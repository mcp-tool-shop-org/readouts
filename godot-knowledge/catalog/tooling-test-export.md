...
- **Verify (solid):** STUDY-025 Verifier ✅ [arXiv 2607.12068 'Beyond Test Presence' (204,673 artifacts): agents show more weak assertions (14.63% vs 11.92%) and unrecognized patterns (11.58% vs 1.46%) vs humans; pass-rate overstates verification quality.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [arXiv 2607.13071 (Tamba, submitted 2026-07-11, cs.SE): killed SIGTERM/exit-143 commands' partial stdout gets recorded as confirmed success in compaction summaries — matches entry's claim almost verbatim.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [arXiv 2602.09311 'Cross-Project Flakiness: A Case Study of the OpenStack Ecosystem' (IEEE TSE, May 2026): 649 projects, 55% affected, 1,156 developer-days lost — matches erosion-of-trust/CI-waste claim.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [arXiv 2605.21677 'A Dataset of Reproducible Flaky-Test Failures' (ReproFlake): 1,115 flaky tests w/ Docker repro images, accepted fixes, logs, PR/issue links — matches entry's dataset claim exactly.]
...
- **Verify (solid):** Remote Scene Tree, breakpoints/Stack Trace/Stack Variables, Errors/Warnings + Network/Video monitors all current 4.x. CLI flags --headless --debug(-d) --verbose(-v) --quit-after N confirmed; --no-window→--headless migration correct. assert() stripped in release builds is accurate. Native-crash-needs-C++-debugger caveat correct. [Godot stable CLI docs confirm -d/--debug, -v/--verbose, --quit-after (iterations), and --headless (=--display-driver headless --audio-driver Dummy) replacing Godot-3's --no-window; assert() fully stripped in release builds.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [arXiv 2207.01047 'Flaky Tests in JavaScript': OS(18.4%)+Platform(10.3%)+Network(12.6%) categories = ~41% of causes are environment-driven, supporting entry's claim (paper doesn't literally say 'Environment').]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [GUT 9.6.0 docs quote verified verbatim: 'When running headless, GUT will automatically ignore all calls to pause_before_teardown and will exit when tests are finished (...you forget -gignore_pause and -gexit).']
...
- **Verify (solid):** STUDY-025 Verifier ✅ [GUT Command-Line docs confirm -gjunit_xml_file ('Export results...to this file in the Junit XML format') and -gjunit_xml_timestamp ('...default false') exactly as named in the entry.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [GUT README states verbatim: 'GUT versions 9.x are for Godot 4.x. GUT versions 7.x (currently 7.4.2) are for Godot 3.x,' plus a full per-release table (9.7.1→4.7.x, 9.6.1→4.6.x, etc).]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [Godot Asset Library asset 1709 = 'GUT - Godot Unit Testing (Godot 4)'; GUT README confirms 9.x=Godot-4.x line, 7.x=Godot-3.x line (7.4.3 current for 3.x). '3.4+' minimum not independently pinned but uncontradicted.]
...
- **Verify (solid):** CORRECTED: Entry's 'what' (exit 0 on error paths) describes issue #85062, not #83449. #83449 is the reverse: exit code 1 despite a successful headless import; fixed in Godot 4.3 via PR #89229 (merged 2024-03-24). · GitHub #83449 is actually 'Exit code 1 after importing headless with --quit' (false FAILURE despite success; fixed by PR #89229 in 4.3) — opposite of entry's 'exits 0 on error', which is #85062's bug. [research note: STUDY-025 Verifier ✅]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [GitHub #85062 'godot cli does not set non zero exit code on error' confirmed: build/export errors silently returned exit 0. Closed 2023 but the general problem persists (open #94957; PR #99254 still unmerged).]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [PR #90431 'Add --import command-line flag' confirmed MERGED 2024-04-09, milestone 4.3: launches editor, waits for imports, quits; --export-release/-debug/-pack now imply --import. Exact match.]
...
- **Verify (solid):** CORRECTED: 'Documented fail switch' overstates status: PR #99254 remains open/unmerged (last updated 2026-08-06, targeting an unreleased future 4.x milestone), not usable in current stable Godot 4.7. · GitHub API confirms PR #99254 state=open, merged=false, milestone '4.x' (future, last updated 2026-08-06). Real proposal, but not merged, shipped, or documented in any stable Godot release as of Sept 2026. [research note: STUDY-025 Verifier ✅]
...
- **Verify (solid):** --export-release/--export-debug/--export-pack/--headless all confirmed in current 4.x CLI docs (--export-debug/--export-pack imply --import). One-preset-per-invocation accurate. --no-window→--headless migration correct. GodotSteam module-vs-GDExtension template rule and steamcmd run_app_build_http + no-auto-publish-to-default-branch are accurate; templates-must-match-editor-version is the real top export failure. [--export-release/-debug/-pack syntax, steamcmd +run_app_build_http (alias of +run_app_build), and GodotSteam module-vs-GDExtension template split verified; open PR #104204 confirms multi-preset export isn't yet stable.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [arXiv 2509.14347 'On the Illusion of Success...Silent Failures in Industrial CI': 142,387+198,146 jobs studied; ignored non-zero exit codes are 18% of public-issue themes — matches entry's silent-failure claim.]
...
- **Verify (solid):** CORRECTED: -gtest is still documented in current GUT 9.x Command-Line docs; -s is Godot's own current --script flag. The real cause of 'Unrecognized options' (GUT issue #150) is using long-form --debug/--script instead of -d/-s, unrelated to version/-gtest. · Core install/API (Asset Library, extends GutTest, test_ prefix, assert_eq/double/stub/spy) confirmed. But -gtest and -s remain CURRENT valid flags in 9.x docs, not deprecated Godot-3 patterns. [research note: Verified on bitwes/Gut: 9.x=Godot 4.x, 7.x=Godot 3.x, MIT license, asset ID 1709 (current 9.6.0 on main, 2026-02-24). extends GutTest, test_*/test_*(), the listed asserts, and double()/stub()/spy all current 4.x API. GdUnit4 correctly named as a separate valid Godot-4 framework (MIT, C#-capable). godot_4 branch / master-references-3 caveat accurate.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [Jest's --passWithNoTests is a real, currently documented CLI/config flag controlling pass/fail on an empty test suite, confirmed via jestjs.io/docs/cli and the jestjs/jest issue tracker.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [arXiv 2607.09345 'How Far Are We from Detecting Flaky Tests? On the Limits of Code-Based Detection' states verbatim 'flakiness is not a static property of test code' — exact match to entry's claim.]
...
- **Verify (solid):** STUDY-025 Verifier ✅ [Open Group Base Specifications Issue 7 (IEEE 1003.1-2017/2008) documents exit-status conventions (127=not found,126=not executable,>128=signal) underpinning the non-zero-fails-unless-ignored pipeline claim.]
...
- **Verify (solid):** Performance.add_custom_monitor(name, Callable) confirmed in current docs (callable must return number >=0; get_custom_monitor to read at runtime); custom monitors are a 4.0+ feature, correctly flagged as a safe 'current' pattern vs Godot 3 which lacked them. Profiler/Monitors tabs accurate. Visual Profiler folding 'in 4.7-dev' is forward-looking but plausible (4.7 is in active dev) and hedged appropriately. [Performance.add_custom_monitor confirmed Godot-4.0+ (Callable-based). Docs confirm Self/Inclusive per-function timing + Idle/Physics breakdown; '4.7-dev tree-folding' only corroborated via a secondary source.]
...
- **Verify (solid):** CORRECTED: As of Sept 2026 current stable is 4.7.2 (released as 4.7 on 2026-06-18), not 4.6.3. 4.6 is still supported but one minor behind; a fresh project starting now should target 4.7.x per the entry's own 'avoid forced migration' logic. · 4.5/4.6 dates confirmed exactly (4.5 support ended 2026-03-19; 4.6 released 2026-01-26; 3.6 sole LTS). But Godot 4.7 shipped 2026-06-18 (now 4.7.2, active support) — 4.6.3 target is one release stale as of Sept 2026. [research note: Every dated fact verified against endoflife.date: 4.6 released 2026-01-26 (latest stable, 4.6.3 on 2026-05-20, active support), 4.5 released 2025-09-15 with regular support ended 2026-03-19 (final 4.5.2), 3.6 the only LTS, no 4.x LTS until 5.0. Godot-3-isms list (CharacterBody2D/await/Node3D/TileMapLayer/@export) all correct. Note:.godot-version is a repo/CI convention not an engine-read file, but the recipe describes it exactly that way (a pin for crew+CI), so it's accurate.]
...
**Notifier isolation: wrapper exit code is a pure function of the child status across failure modes — exit-contract craft against silent/wrong greens for recipe 49.**
- **How:** Literature deepen for leftover 37/42/49; no verified flip.
- **Gotchas:** STUDY-044 deepen note.
- **Verify (shaky):** STUDY-044 deepen; default unverified [no external verdict — not checked]
- **Sources:** [GPUAlert: A Zero-Instrumentation Process-Boundary Monitor for Diagnosing GPU Tra](https://arxiv.org/abs/2607.01409) — Notifier isolation: wrapper exit code is a pure function of the child status across failure modes — exit-contract craft against silent/wrong greens for recipe 49.

### GUT Command Line (official pair for 49) · `⚠ shaky` · Godot 4.x
**Official GUT CLI documents -gexit and -gexit_on_success only — no -gexit_on_complete. Documents **`-gexit`** and **`-gexit_on_success` only**. **No `-gexit_on_complete`.** Returns 0 on pass / 1 on fail. **Hard mismatch with blog (4):** invented/wrong exit flag → false-green risk. verified=0 stays for 49.**
- **How:** URL-open / official support deepen; 37/42/49 verified=0 stays.
- **Gotchas:** STUDY-044 deepen note.
- **Verify (shaky):** STUDY-044 deepen; default unverified [no external verdict — not checked]
- **Sources:** [GUT Command Line (official pair for 49)](https://gut.readthedocs.io/en/latest/Command-Line.html) — Official GUT CLI documents -gexit and -gexit_on_success only — no -gexit_on_complete. Documents **`-gexit`** and **`-gexit_on_success` only**. **No `-gexit_on_complete`.** Ret
...
**Flaky tests pass/fail on same version and create false alerts; CI must separate flaky failures from fault-triggering ones — false-signal CI frame for recipe 49.**
- **How:** Literature deepen for leftover 37/42/49; no verified flip.
- **Gotchas:** STUDY-044 deepen note.
- **Verify (shaky):** STUDY-044 deepen; default unverified [no external verdict — not checked]
- **Sources:** [The Importance of Discerning Flaky from Fault-triggering Test Failures: A Case S](https://arxiv.org/abs/2302.10594) — Flaky tests pass/fail on same version and create false alerts; CI must separate flaky failures from fault-triggering ones — false-signal CI frame for recipe 49.

### Understanding and Detecting Flaky Builds in GitHub Actions · `⚠ shaky` · Godot 4.x
**Large-scale GHA study: reruns; 67.73% of rerun builds flaky (outcome changes without code change) — unreliable green/rerun CI for recipe 49 deepen (omit STUDY-025 eight).**
- **How:** Literature deepen for leftover 37/42/49; no verified flip.
- **Gotchas:** STUDY-044 deepen note.
- **Verify (shaky):** STUDY-044 deepen; default unverified [no external verdict — not checked]
- **Sources:** [Understanding and Detecting Flaky Builds in GitHub Actions](https://arxiv.org/abs/2602.02307) — Large-scale GHA study: reruns; 67.73% of rerun builds flaky (outcome changes without code change) — unreliable green/rerun CI for recipe 49 deepen (omit STUDY-025 eight).
...
**CLI documents -gexit / -gexit_on_success only (no -gexit_on_complete); GUT 9.7.1 targets Godot 4.7.x.**
- **How:** See source URL; STUDY-002 Verifier-verified finding.
- **Verify ():** STUDY-012 from STUDY-002 Verifier ✅; default verified=0 [GUT's current Command-Line docs (Run Options) list only -gexit and -gexit_on_success — no -gexit_on_complete anywhere. GUT 9.7.1 (released 2026-07-10) targets Godot 4.7, confirming the version claim too.]
- **Sources:** [GUT CLI -gexit / -gexit_on_success (4.7.x)](https://gut.readthedocs.io/en/latest/Command-Line.html) — official GUT exit flags
...
- **How:** See source URL; STUDY-002 Verifier-verified finding.
- **Verify ():** STUDY-012 from STUDY-002 Verifier ✅; default verified=0 [arXiv 2606.17861 'GameCraft-Bench': 140 Godot tasks/15 families; strongest agent scores 41.46% (exact match); paper states Godot chosen for text scenes + headless CLI + native 2D reproducible grounding.]
...
- **How:** See source URL; STUDY-002 Verifier-verified finding.
- **Verify ():** STUDY-012 from STUDY-002 Verifier ✅; default verified=0 [arXiv 2602.11103 'GameDevBench': 333 tasks (154 core+179 variants) sourced from Godot 4 tutorials; success 51.4% gameplay vs 33.0% 2D-graphics — matches entry's ~51%/~33% split precisely.]
...
- **How:** See source URL; STUDY-002 Verifier-verified finding.
- **Verify ():** STUDY-012 from STUDY-002 Verifier ✅; default verified=0 [gradle/gradle#8739 confirmed: System.exit(0) in test code lets the JVM exit clean, so Gradle reports BUILD SUCCESSFUL though not all tests ran — exact analog for an invented exit-early flag producing false-green CI.]
- **Sources:** [Gradle #8739 System.exit false-green analog](https://github.com/gradle/gradle/issues/8739) — false-green exit
...
- **How:** See source URL; STUDY-002 Verifier-verified finding.
- **Verify ():** STUDY-012 from STUDY-002 Verifier ✅; default verified=0 [arXiv 2606.19830 'JAMER': deterministic verification pipeline distills 8,133 verified projects from 240,000+ repos, built on Godot's text-based format + headless execution — exact number and mechanism match.]
...
- **How:** See source URL; STUDY-002 Verifier-verified finding.
- **Verify ():** STUDY-012 from STUDY-002 Verifier ✅; default verified=0 [pytest.ExitCode enum confirmed: 0=all passed,1=some failed,2=interrupted,3=internal error,4=usage error,5=no tests collected — a real, public, documented exit-code contract supporting the no-invented-codes analogy.]
- **Sources:** [Pytest exit codes — named exit contract analog](https://docs.pytest.org/en/stable/reference/exit-codes.html) — documented exit flags
