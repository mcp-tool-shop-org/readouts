#!/usr/bin/env python3
"""regen.py — regenerate EVERY derived artifact from godot.db.
...
The pillars rebuilt from the single source of truth (godot.db):
  data    : godot.db                             (load_db.py ingests each wave's research-raw.json)
  ...
  output  : readout/*.html + index.{md,json}      (shared gen_readout.py / gen_index.py) — best-effort,
            skipped: godot.db has no readout profile (a lean catalog+loadout KB, like sprites/docker).
...
def main():
    ...
    # 4. shared readout/index/reports — profile-driven; best-effort (skips: godot.db has no profile)
    ...
    print("\n✓ regen complete — artifacts rebuilt from godot.db")
