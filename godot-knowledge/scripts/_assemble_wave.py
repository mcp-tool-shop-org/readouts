#!/usr/bin/env python3
"""Assemble a godot-knowledge wave swarm output ({research, verify}) into the load_db
...
def main(path, wave_no=1, date="2026-06-18"):
    ...
    for i, r in enumerate(research):
        ...
        for rec in (r.get("recipes") or []):
            ...
            recipes.append({
                ...
                "what": rec.get("what"),
                "how": rec.get("how"),
                "godot_version": rec.get("godot_version"),
    ...
    out = {
        "wave": wave_no,
        "date": date,
        "title": "Wave 1 — Godot-4 tactical-RPG dev knowledge (study swarm)",
        "domain_scope": ", ".join(LANE_SLUGS),
        "agent_count": agent_count,
        "verifier_note": (f"6 web-grounded Godot-4 research lanes, each adversarially verified against current "
                          f"Godot 4.x docs for CURRENCY (flags Godot-3 staleness, deprecated nodes, version-wrong "
