...
- [x] `[all]` No known high/critical vulnerabilities in any dependency tree, and Dependabot alerts are enabled — executed by `npx @mcptoolshop/shipcheck deps` (the OUTCOME: audits **every** tree incl. subtrees, not just the root; `ci` only proves a scanner is configured) (2026-09-25; `shipcheck deps` run with a token: 1 tree audited, no vulnerabilities at high or above, and Dependabot vulnerability alerts enabled for mcp-tool-shop-org/readouts. The owner enabled them the same day; the API answers 204)
