...
- **Verify:** CORRECTED: Apache-2.0 on both code (github.com/zjp-shadow/CharacterGen LICENSE, plain Apache 2.0 with no added clauses) and weights (HF zjpshadow/CharacterGen, license: apache-2.0). Venue is SIGGRAPH / ACM TOG 2024. The multi-view stage produces FOUR canonicalized views, not a three-view front/side/back set; the project page says only that it 'calibrates input poses to a canonical form' — no retrieved source says A-pose, so drop that word. The VRoid/VRM training data cannot be redistributed ('Due to the policy, we cannot redistribute the raw data of VRM format 3D character'), so the dataset hedge is justified. Drop the superlative: it is NOT the only commercially licensed character-turnaround model — Onodofthenorth's sprite-sheet checkpoint is apache-2.0 and Chamber's Charturn LoRAs grant Sell on Civitai. The diffusion backbone version is unstated in the repo, the paper page and the model card, so the 'older/lower-fidelity than SDXL' comparison is unverified. · Apache-2.0 confirmed twice: repo LICENSE and HF zjpshadow/CharacterGen licence field. SIGGRAPH(TOG) 2024; canonicalization and TripoSR real. But the 2D stage emits FOUR views, 'A-pose' is stated nowhere, and raw VRM data is non-redistributa [research note: Both sources resolve. arXiv:2402.17214 title/authors/multi-view pose canonicalization for anime characters confirmed. GitHub zjp-shadow/CharacterGen confirmed Apache-2.0, official SIGGRAPH'24 (TOG), built on Tune-A-Video + TripoSR, anime VRM dataset. Note: VRM raw data cannot be redistributed per policy (repo flags this) and weights are trained on it — the entry's 'verify dataset terms for weight redistribution' caveat is well-founded. Code/commercial_use=yes is accurate for the Apache-2.0 code/weights.]
...
- **Verify:** CORRECTED: Zero123C is not a separate repo — it is the file stable_zero123_c.ckpt (8.58 GB) sitting alongside stable_zero123.ckpt inside stabilityai/stable-zero123, each with its own licence file. The default checkpoint is licensed sai-nc-community (Stability AI Non-Commercial Research Community License) because it 'included some CC-BY-NC 3D objects'. The C checkpoint's LICENSE_stable_zero123_c.md is titled 'STABILITY AI COMMUNITY LICENSE AGREEMENT' and grants free commercial use until 'You or Your Affiliate(s) ... generate more than USD $1,000,000 in annual revenue', at which point the licence terminates and an Enterprise licence must be requested. The 'conditional commercial under a Stability membership' framing is obsolete — the membership scheme was superseded by the revenue-threshold Community License in July 2024. The check is a revenue tier, not a membership status. · Both checkpoints confirmed inside ONE repo: stable_zero123.ckpt (sai-nc-community, NC research) and stable_zero123_c.ckpt. But LICENSE_stable_zero123_c.md is the STABILITY AI COMMUNITY LICENSE — free commercial below USD $1,000,000 annual r [research note: Both sources resolve. HF stabilityai/stable-zero123 confirms standard Stable Zero123 uses CC-BY-NC objects under Stability AI Non-Commercial Research Community License (strictly non-commercial), finetuned from lambdalabs/sd-image-variations-diffusers. Blog confirms Zero123C trained only on CC-BY/CC0 for commercial use. The bundled 'conditional' label is fair: standard variant is non-commercial, Zero123C is commercial under the Community License (blog notes commercial use tied to active Stability membership). Accurate.]
...
- **Verify:** CORRECTED: Code is AGPL-3.0 — GitHub's licence API reports SPDX agpl-3.0 in a file named LICENCE (British spelling, which is why a raw /LICENSE fetch 404s). The abstract states Era3D generates images 'with up to a 512*512 resolution', and the released checkpoint is pengHTYX/MacLab-Era3D-512-6view: 512x512 across 6 views, NOT 1024x512, and therefore not the highest native resolution in this set. That HF weights repo is tagged license: apache-2.0, which directly conflicts with the AGPL code licence; treat the code as the binding copyleft trap and do not assume the weights tag rescues a closed pipeline. The orthographic variant is real and confirmed from the README (MacLab-Era3D-512-6view-ortho, 'ensuring consistency between input and generated front-view images'), as is NeurIPS 2024 acceptance. Last notable release: Oct 2024. · AGPL-3.0 on the CODE confirmed via GitHub licence API (SPDX agpl-3.0, file 'LICENCE'). But resolution is 512x512 max, not 1024x512 — so it is NOT the highest-res entry — and the HF weights repo is tagged apache-2.0, contradicting 'code/weig [research note: Both sources resolve. GitHub pengHTYX/Era3D confirmed official, AGPL-3.0 repo AND pretrained model (downstream must open-source) — license and commercial_use=no are CORRECT (decisive axis verified). Minor non-license inaccuracies: paper abstract states up to 512x512 (HF model is MacLab-Era3D-512-6view), not the claimed '1024x512'; the abstract surfaced does not mention the 'orthographic-camera variant.' Row-wise attention 12x compute cut and NeurIPS 2024 confirmed. Verified on license; flag the resolution/orthographic claims for correction.]
...
- **Verify:** CORRECTED: The authors' own README is unambiguous — 'The code is released under Apache 2.0 and the model weights are released under CC-BY-NC 4.0 ... you cannot use the model (or its derivatives) in a commercial product pipeline, but you can still use the outputs from the model freely' — so the do-not-ship conclusion stands and commercial_use: no is right. What is wrong is the implication of a single clean identifier: the HuggingFace card for sudo-ai/zero123plus-v1.1 declares license: openrail, and sudo-ai/zero123plus-v1.2 publishes no licence field at all. Record the conflict in provenance rather than one string, and treat the author-written README as binding because it is the stricter reading. Note also the usable half the recipe understates: OUTPUTS are explicitly free to use even though the weights are not. The ~5GB / ~5.7GB VRAM figures are README-stated with no hardware named, so they are indicative only. · Repo README states verbatim 'code is released under Apache 2.0 and the model weights are released under CC-BY-NC 4.0' — but HF sudo-ai/zero123plus-v1.1 is tagged 'openrail' and v1.2 shows no licence field at all. Contradictory. VRAM ~5GB, ~ [research note: Both sources resolve. GitHub SUDO-AI-3D/zero123plus confirms code Apache-2.0, weights CC-BY-NC-4.0 (non-commercial), 6 fixed views, ~5GB VRAM (~5.7GB with depth ControlNet sudo-ai/controlnet-zp11-depth-v1). arXiv:2310.15110 title/authors confirmed. License split and commercial_use=no are accurate — weights are non-commercial.]
