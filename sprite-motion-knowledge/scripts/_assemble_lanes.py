...
def main():
    ...
    rec = sum(1 for l in lanes for r in l["recipes"] if (r.get("status") or "").lower() == "recommended")
    print(f"assembled {op}: {len(lanes)} lanes, {n} recipes ({rec} self-rated recommended)")
