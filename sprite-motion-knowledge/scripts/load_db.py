...
# agents invent status values; normalize to the canonical enum so v_recommended + ordering stay correct
STATUS_MAP = {
    "recommended": "recommended", "required": "recommended", "recommend": "recommended",
    "runner-up": "runner-up", "runnerup": "runner-up", "viable": "runner-up", "alternative": "runner-up",
    "situational": "situational", "conditional": "situational", "conditional-recommended": "situational",
    "evaluate": "situational", "reference": "situational", "experimental": "situational",
    "legacy": "legacy", "superseded": "superseded",
    "avoid": "avoid", "blocked-license": "avoid", "blocked": "avoid", "do-not-use": "avoid",
    "avoid-for-ai": "avoid", "non-commercial": "avoid", "rejected": "avoid",
}


def norm_status(s):
    """Map an agent's free-text status to the canonical enum (unknown non-empty -> situational)."""
    k = (s or "").strip().lower()
    if not k:
        return s
    return STATUS_MAP.get(k, "situational")
...
def _dl(items, key):
    """Tolerate agents that emit a companion list as bare strings: 'x' -> {key: 'x'} (load-bearing — wave
    agents sometimes return failures/purposes/hparams as plain string lists instead of objects)."""
    return [(it if isinstance(it, dict) else {key: it}) for it in (items or [])]
...
def main(path):
    ...
    for lane in lanes:
        ...
        for t in (lane.get("recipes") or []):
            for _k in ("slug", "name", "kind", "engine_family", "applicable_to", "base_model_family", "claim",
                       "summary", "design_implication", "evidence_strength", "seed", "variance_note",
                       "tuning_budget", "measured_conditions", "engine_recipe_ref", "base_model_slug", "license",
                       "commercial_use", "commercial_notes", "vram_gb", "status", "verify_note", "license_correction"):
                if isinstance(t.get(_k), list):  # tolerate agents that emit a scalar field as a list
                    t[_k] = ", ".join(str(x) for x in t[_k])
            name = t.get("name")
            ...
            for hp in _dl(t.get("hparams"), "name"):
                ...
                n_hp += 1
            for fa in _dl(t.get("failures"), "symptom"):
                cur.execute(
                ...
                n_fail += 1
            for ev_row in _dl(t.get("evals"), "note"):
                cur.execute(
                ...
                n_eval += 1
            for pu in _dl(t.get("purposes"), "purpose_name"):
    ...
    meta = {
        "kb_name": "sprite-motion-knowledge",
        "rig": "OMEN 45L · RTX 5090 · Blackwell sm_120 · 32 GB VRAM · Core Ultra 9 · 64 GB RAM · Windows 11 / WSL2 (the only machine)",
        "scope": "The sprite-MOTION / character-animation craft — recipes (procedures + model/tool choices) that "
                 "animate the 2.5D JRPG sprites produced by sprites-knowledge (KB #5): the rig-truth-first "
                 "architecture, rigging & skeletons, AI motion models, inbetweening, cloud GPU workers, combat "
                 "animation craft, and motion QA. The spine is motion truth (rig/mesh/proxy) -> AI polish -> "
                 "sprite-sheet export -> local verify — the same insight the studio proved for rigid weapons, "
                 "extended to walk/attack motion. Proven (measured-on-rig) vs research (study-swarm sourced) is "
                 "encoded by evidence_strength; license (commercial_use) is the second decisive axis. Sibling KBs "
                 "own the layers this one points at (model-knowledge weights, tensor-engine software, "
                 "sprites-knowledge static-sprite craft, blender-knowledge Blender recipes); never restated.",
    }
