...
The drift class (caught in the sibling tensor-engine-knowledge KB, where meta
froze at wave 4 while the DB grew to 14): load_db.py stamped
`latest_wave = str(wave_no)` from the wave being ingested, so any wave appended
by a one-off script — or any re-ingest of an older wave — left meta lying about
how current the KB is. This matters acutely for docker-knowledge: it backs the
gpu-container product, whose measured-result write-back must not inherit a
provenance pointer that lies about which wave it measured against.

The cure: never trust the ingest wave; always recompute from the waves table.
...
set_meta_currency is imported by load_db.py (root-cause fix at ingest time). This
file also runs standalone (set + verify, exits non-zero on drift) — run it after
any wave append. (This KB has no regen.py yet; when one is added, make this a
non-skippable first step + a final verify, mirroring the sibling KBs.)
...
def verify_meta(cur):
    ...
    The guard that makes silent drift impossible: any path that grows the waves
    table without refreshing meta trips this on the next run."""
    ...
    if meta_val != str(wave_no):
        ...
        raise MetaDriftError(
            ...
            f"(stale by {behind} wave(s)). Run scripts/refresh_meta.py to correct it."
        )
