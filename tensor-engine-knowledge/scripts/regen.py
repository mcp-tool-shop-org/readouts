#!/usr/bin/env python3
"""regen.py — regenerate EVERY derived artifact from engines.db. Run after a wave's load_db.py.

The four pillars rebuilt from the single source of truth (engines.db):
  data    : engines.db                         (load_db.py ingests a wave's research-raw.json — run separately)
  catalog : catalog/*.md                       (gen_catalog.py)
  output  : readout/*.html + index.{md,json}   (gen_readout.py [all domains] + gen_index.py)   <- this KB's product face
...
def main():
    ...
    # Fails the whole regen loudly rather than letting the KB ship a lying pointer.
    ...
    try:
        ...
        con.close()
    print("  ok")
    print("\n✓ regen complete — all four pillars rebuilt from engines.db")
