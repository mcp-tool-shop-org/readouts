...
History of the bug this fixes: waves 5-14 were ingested by one-off `_waveN_*.py`
scripts that append to `waves`/`engines` but never write `meta`. Only load_db.py
wrote meta, and it stamped `latest_wave = str(wave_no)` from the wave it was
ingesting — so meta froze at wave 4 while the DB grew to 14. The provenance
pointer lied by ten waves.
