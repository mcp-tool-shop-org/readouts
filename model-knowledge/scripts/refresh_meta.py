...
how current the KB is. model-knowledge has wave 0 (the local-workflows seed), so
"latest" is correctly MAX(wave_number), never the row count.
...
Both are importable (load_db.py calls set_meta_currency inside its transaction so
the root cause is fixed at ingest time) and this file runs standalone as a
non-skippable regen step (set + verify, exits non-zero on drift).
...
def verify_meta(cur):
    ...
    table without refreshing meta trips this on the next regen."""
