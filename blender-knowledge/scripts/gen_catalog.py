#!/usr/bin/env python3
"""Generate human-readable catalog/*.md from blender.db. Re-run after each wave:
...
CUR = {"solid": "✅ solid", "plausible": "▸ plausible", "shaky": "⚠ shaky",
       "blender3_stale": "⛔ blender-3 stale", "wrong": "✗ wrong"}
...
def write_cat(c, cat, wave, date):
    ...
    L = [f"# {cat['name']}",
         ...
         "|--------|-------|----------|---|------|"]
    for t in rows:
        L.append(f"| {cell(t['name'])} | {cell(t['blender_version'])} | {CUR.get(t['currency'], cell(t['currency']))} | "
                 f"{'✓' if t['verified'] else '·'} | {cell(t['what'])[:80]} |")
    L.append("\n## Detail\n")
    for t in rows:
        L.append(f"### {t['name']} · `{CUR.get(t['currency'], t['currency'] or '?')}` · Blender {cell(t['blender_version'])}")
        if t["what"]:
            L.append(f"**{t['what']}**")
        if t["how"]:
        ...
        if t["studio_use"]:
            L.append(f"- **For Studio:** {t['studio_use']}")
        if t["verify_note"]:
...
def main():
    ...
    idx = [
        "# Catalog — Blender 4.x for the studio's headless render & game-asset pipeline\n",
        f"Generated from `blender.db` · wave {wave} · {date}. NEVER hand-edited — regenerated from the DB.\n",
        f"Current, adversarially-verified **Blender 4.x** practice for the studio's headless sprite-turnaround render "
        f"pipeline (import a TRELLIS GLB → render 8-direction sprites → 2.5D game) plus game-asset prep. {total} "
        f"recipes · {ver} verified. Decisive axis = **Blender-4.x currency** (solid → wrong); the verifier flags "
        f"Blender-3 staleness. Sibling KBs in this monorepo cover the models / engines / training / sprites / godot.\n"]
    ...
    if flagged:
        ...
        idx += ["_None flagged this wave — every recipe verified solid/plausible against current Blender 4.x._\n"]
    ...
    idx += ["", "## Legend\n",
            "- **Currency** (the decisive axis): ✅ solid > ▸ plausible > ⚠ shaky > ⛔ blender-3 stale > ✗ wrong. The "
            "verifier web-checked each recipe against current Blender 4.x and flags Blender-3-isms (deprecated nodes, "
            "renamed APIs).",
            "- **✓** verified (currency solid/plausible). **Blender** = the version the recipe targets (e.g. 4.x, 4.3+).",
