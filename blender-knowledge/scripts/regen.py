#!/usr/bin/env python3
"""regen.py — regenerate EVERY derived artifact from blender.db.
...
The pillars rebuilt from the single source of truth (blender.db):
  data    : blender.db                             (load_db.py ingests each wave's research-raw.json)
  ...
  output  : readout/*.html + index.{md,json}      (shared gen_readout.py / gen_index.py) — best-effort,
            skipped: blender.db has no readout profile (a lean catalog+loadout KB, like sprites/docker).
...
def main():
    ...
    # 4. shared readout/index/reports — profile-driven; best-effort (skips: blender.db has no profile)
    ...
    print("\n✓ regen complete — artifacts rebuilt from blender.db")
